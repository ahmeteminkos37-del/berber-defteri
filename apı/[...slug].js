...
function publicShop(s) {
  ...
  return { id: s.id, name: s.name, open: s.open, close: s.close, prices: shopPrices(s), otpRequired: OTP_REQUIRED, staff: staff.map(pubStaff) };
}
...
function ownerShopView(s) {
  return { id: s.id, name: s.name, username: s.username || '', open: s.open, close: s.close, prices: shopPrices(s),
    plan: s.plan || 'trial', expiresAt: s.expires_at || null, daysLeft: daysLeft(s), expired: !isLive(s), paymentMode: PAYMENT, credits: s.credits || 0, couponOff: !!s.coupon_off, city: s.city || '', district: s.district || '',
...
async function getPlatform() {
  const { data } = await supabase.from('platform').select('fee,admin_pw,packages,credit_packages,hair_models,hair_price,referrers,theme,style,coupon_enabled').eq('id', 1).single();
...
async function config() { const pl = await getPlatform(); return { fee: pl.fee, packages: pl.packages, paymentMode: PAYMENT, creditPackages: pl.creditPackages, hairModels: pl.hairModels.map(m => ({ id: m.id, name: m.name })), hairPrice: pl.hairPrice, theme: pl.theme, style: pl.style, couponEnabled: pl.couponEnabled }; }
...
async function createShop(res, P) {
  ...
  if (staff.some(s => !isPhone(s.phone))) return send(res, 400, { error: 'Her çalışan için geçerli WhatsApp numarası girin.', field: 'staff' });
  ...
  let ref_code = '';
  const { error } = await supabase.from('shops').insert({ id, name, username, pw: hashPw(pw), open: P.open, close: P.close, step: GRAN, staff, prices: sanitizePrices(P.prices), closed_days: [], credits: HAIR_GIFT, ref_code, city, district, plan: 'active', expires_at: new Date(Date.now() - 1000).toISOString(), status: 'active' });
...
async function updateShop(res, sid, P) {
  ...
  if (staff.some(s => !isPhone(s.phone))) return send(res, 400, { error: 'Her çalışan için geçerli WhatsApp numarası girin.', field: 'staff' });
  const upd = { name: String(P.name || shop.name).trim(), open: P.open, close: P.close, staff, prices: sanitizePrices(P.prices) };
...
async function ownerStats(res, sid) {
  ...
  const { data } = await supabase.from('appointments').select('date,time,dur,price,staff_id').eq('shop_id', sid);
...
async function overview(res) {
  ...
  send(res, 200, { fee: pl.fee, totalAppts: totalAppts || 0, shops: out });
}
...
async function resetStart(res, P) {
  const u = String(P.username || '').trim().toLowerCase();
  const { data: shop } = await supabase.from('shops').select('id,staff').ilike('username', u).maybeSingle();
  const phone = shop && (shop.staff || [])[0] && shop.staff[0].phone;
  if (!shop || !phone) return send(res, 404, { error: 'Bu kullanıcı adına kayıtlı doğrulanabilir numara bulunamadı.' });
  const code = String(Math.floor(1000 + Math.random() * 9000));
  const otpToken = signToken({ t: 'reset', sid: shop.id, phone: normPhone(phone), ch: sha(code + SECRET), exp: Date.now() + 10 * 60000 });
  if (SMS_PROVIDER === 'mock') return send(res, 200, { otpToken, maskedPhone: maskPhone(phone), devCode: code });
  try { await sendSms(phone, 'Sifre sifirlama kodunuz: ' + code); }
  catch (e) { return send(res, 502, { error: 'SMS gonderilemedi, biraz sonra tekrar deneyin.' }); }
  send(res, 200, { otpToken, maskedPhone: maskPhone(phone) });
}
async function resetFinish(res, P) {
  ...
  const tok = verifyToken(P.otpToken);
  if (!tok || tok.t !== 'reset') return send(res, 400, { error: 'Kod süresi doldu, baştan deneyin.' });
  if (tok.ch !== sha(String(P.otp || '') + SECRET)) return send(res, 400, { error: 'Kod hatalı.' });
  const { data: shop } = await supabase.from('shops').select('id').ilike('username', u).maybeSingle();
  if (!shop || shop.id !== tok.sid) return send(res, 400, { error: 'Geçersiz istek.' });
